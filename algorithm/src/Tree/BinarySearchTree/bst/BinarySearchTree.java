package Tree.BinarySearchTree.bst;
...
public class BinarySearchTree {
    ...
    private Node insertNode(Node x, Node newNode) {
        /**
         * 1. x 노드가 null일 때 까지 x 노드의 키와 새로운 노드의 키를 비교한다.
    ...
    private Node removeNode(Node x, int key) {
         ...
         * 2-1-2 삭제할 키와 가장 큰 키를 swap 한다
         * 2-1-3 삭제할 노드의 좌측 자식 부터 다시 탐색 (1로 이동)
         * 2-2 우측 노드가 null이 아니다
         * 2-2-1 삭제할 노드의 우측 서브 트리 중 가장 작은 키(successor key)를 가진 노드를 찾는다.
         * 2-2-2 삭제할 키와 가장 작은 키와 swap 한다
         * 2-2-3 삭제할 노드의 우측 자식 부터 다시 탐색 (1로 이동)
