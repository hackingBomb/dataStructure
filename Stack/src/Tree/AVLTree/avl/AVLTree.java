...
public class AVLTree {
    ...
    private Node insertNode(Node x, Node newNode) {
        /**
         * 이진 탐색 트리와 동일
         * 1. x 노드가 null일 때 까지 x 노드의 데이터와 새로운 노드의 데이터를 비교한다.
         * 1-1 x 노드의 데이터보다 작으면 좌측 노드로 이동한다
         * 1-1-1 x 노드의 좌측 노드로 이동한다. (1로 이동)
         * 1-2 x 노드의 데이터보다 크면 우측 노드로 이동한다
         * 1-2-1 x 노드의 우측 노드로 이동한다. (1로 이동)
         * 2. x가 null 이면 새로운 노드를 반환한다
         * 3. 이전 분기점으로 돌아가는 방식으로 x 노드 부터 높이를 재조정한다.
         * 4. 균형도가 맞지 않으면 회전시킨다.
         */
        if (null == x) {
            return newNode;
    ...
    private Node removeNode(Node x, int key) {
        ...
        if (null == x) {
            ...
            if (null != x.left) {
                ...
                // TODO : 삭제할 키와 좌측 서브 트리 노드중 가장 큰 키와 Swap
                int removeKey = x.key;
                x.key = predecessor.key;
                predecessor.key = removeKey;
                // TODO : 삭제할 노드의 좌측 자식 부터 다시 탐색 (1로 이동)
                x.left = removeNode(x.left, key);
                ...
                // TODO : 삭제할 노드의 우측 자식 부터 다시 탐색 (1로 이동)
                x.right = removeNode(x.right, key);
            } else {
                // TODO: null을 반환함으로 연결을 끊는다
                return null;
            }
        }
        // TODO : 이전 분기점으로 돌아가는 방식으로 height 조정 후 회전
    ...
    private Node rotate(Node x) {
        ...
        if (Math.abs(xNodeBalance) >= 2) {
            // TODO : 회전으로 인한 변경된 새로운 x 노드를 반환한다.
